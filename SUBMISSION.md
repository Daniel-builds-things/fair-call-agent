...
**Fair Call Agent** is an agentic constraint-reasoning layer built on top of Fair Call Pro — a production shift scheduling app originally created for healthcare shift distribution, now industry-agnostic and used by clinics, security firms, call centers, and more.

### The Origin Story
Fair Call Pro started as a practical tool built during a medical laboratory science internship — solving the real problem of distributing shifts and calls fairly across a team. It worked well, but it was completely deterministic: everyone was treated as interchangeable. The moment someone said "I need time off" or "I can't work weekends," the system had no idea what to do. And the problem wasn't unique to healthcare — it exists in every industry that schedules people.

So we took it further: we built an agentic layer that uses an LLM to understand natural language constraints, reason about them, and produce schedules that are fair AND practical.
